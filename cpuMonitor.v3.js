// Here we want to register long tasks in order to expose result to the tracking library
window.__longTasksTracker__ = {
    track: function () {
        if (window.PerformanceObserver) {
            var longTasksHandler = function (perfList) {
                this.longTasks = this.longTasks.concat(perfList.getEntries());
            };
            var observer = new window.PerformanceObserver(longTasksHandler.bind(this));
            observer.observe({
                entryTypes: ['longtask'],
            });
        }
...
window.__longTasksTracker__.track();
                ...
                c.apply(null, [].concat(fA)).then(
                    ...
                    function (a) {
                        if (
                            ...
                            fA.length >= 2 &&
                            fA[0] &&
                            typeof fA[0].split === 'function' &&
                            fA[1]
                        ) {
                            a.message += ` (${fA[1].method} ${fA[0].split('?')[0]})`;
                        }
                        b(a);
                        e(a);
                    }
...
var ttiFunc = function (p) {
    p.getFirstConsistentlyInteractive().then(function (tti) {
        if (window.Omnitracking && typeof Omnitracking.track === 'function' && tti !== null) {
            var ttiFinal = Math.round(tti);
            Omnitracking.track({
                tid: 1217,
                val: `{"tti":${ttiFinal}}`,
            });

            if (!!CoralogixRum && CoralogixRum.isInited) {
                window.CoralogixRum.info('Time To Interactive', {
                    tti: ttiFinal,
                });
            }
        }
...
if (typeof require === 'function') {
    require(['ttiPolyfill'], ttiFunc);
} else if (window['ttiPolyfill']) {
    ttiFunc(window['ttiPolyfill']);
}
